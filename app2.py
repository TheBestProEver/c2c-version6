...
use_fake_names = st.sidebar.toggle("Real-world Sample Profiles", value=True)
...
# 5. Core Engine with Safety Fallbacks
def run_matching_engine(n_patients, n_doctors, r_seed, fake_names_flag):
    ...
    # High capacity buffer to prevent unsolvable scenarios
    doctors = pd.DataFrame({
        ...
        'Capacity': np.random.randint(8, 15, n_doctors)
    })
    ...
    for i in patients.index:
        for j in doctors.index:
            if x[i, j].varValue is not None and x[i, j].varValue == 1.0:
                primary_matches[i] = j
    ...
    for i in patients.index:
        ...
        for j in doctors.index:
            ...
            if p_rec['Insurance'] in d_rec['Accepted_Insurances']:
                ...
                compat.append({
                    ...
                    'Is_Primary': (primary_matches.get(i) == j)
                })
        ...
        if len(compat) > 0:
            ...
            backups = [c for c in compat if c != primary][:2]
            ...
            p_clinic = primary['Clinic_ID']
            p_dist = primary['Distance']
            b1 = f"{backups[0]['Clinic_ID']} ({backups[0]['Distance']} mi)" if len(backups) > 0 else "N/A"
            b2 = f"{backups[1]['Clinic_ID']} ({backups[1]['Distance']} mi)" if len(backups) > 1 else "N/A"
            all_b = [f"{b['Clinic_ID']} ({b['Distance']} mi)" for b in backups]
        else:
            p_clinic, p_dist, b1, b2, all_b = "No Network Clinic", 0.0, "N/A", "N/A", []
        ...
        results.append({
            ...
            'Primary_Clinic': p_clinic,
            'Primary_Distance': p_dist,
            'Backup_1': b1,
            'Backup_2': b2,
            'All_Backups': all_b
        })
